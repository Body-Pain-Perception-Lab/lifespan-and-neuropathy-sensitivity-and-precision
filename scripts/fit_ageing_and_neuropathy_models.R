# Script to fit the different models the ageing and neuropathy dataset.
...
if (!dir.exists("results/fits/ageing_and_neuropathy")) {
  dir.create("results/fits/ageing_and_neuropathy", recursive = TRUE)
}
if (!dir.exists("results/loo/ageing_and_neuropathy")) {
  dir.create("results/loo/ageing_and_neuropathy", recursive = TRUE)
}
#### Define function ####
fit_model<-function(parameters){
  mod<-cmdstan_model(stan_file=paste(parameters$directory,"/stan_models/ageing_and_neuropathy/",parameters$model,sep=""),
  ...
  fit$save_object(paste0(parameters$directory,"/results/fits/ageing_and_neuropathy/", str_remove_all(parameters$model,".stan"), ".rds"))

  loo<-fit$loo(cores=4,variables='log_lik')
  saveRDS(loo,paste0(parameters$directory,"/results/loo/ageing_and_neuropathy/", str_remove_all(parameters$model,".stan"), ".rds"))
  rm(loo)
  gc()
  loo<-fit$loo(cores=4,variables='log_lik',moment_match=T)
  saveRDS(loo,paste0(parameters$directory,"/results/loo/ageing_and_neuropathy/MM_", str_remove_all(parameters$model,".stan"), ".rds"))
  rm(loo)
  gc()

  loo<-fit$loo(cores=4,variables='log_lik_cdt')
  saveRDS(loo,paste0(parameters$directory,"/results/loo/ageing_and_neuropathy/cdt_", str_remove_all(parameters$model,".stan"), ".rds"))
  rm(loo)
  gc()
  loo<-fit$loo(cores=4,variables='log_lik_cdt',moment_match=T)
  saveRDS(loo,paste0(parameters$directory,"/results/loo/ageing_and_neuropathy/MM_cdt_", str_remove_all(parameters$model,".stan"), ".rds"))
  rm(loo)
  gc()
  loo<-fit$loo(cores=4,variables='log_lik_wdt')
  saveRDS(loo,paste0(parameters$directory,"/results/loo/ageing_and_neuropathy/wdt_", str_remove_all(parameters$model,".stan"), ".rds"))
  rm(loo)
  gc()
  loo<-fit$loo(cores=4,variables='log_lik_wdt',moment_match=T)
  saveRDS(loo,paste0(parameters$directory,"/results/loo/ageing_and_neuropathy/MM_wdt_", str_remove_all(parameters$model,".stan"), ".rds"))
  rm(loo)
  gc()
  loo<-fit$loo(cores=4,variables='log_lik_cpt')
  saveRDS(loo,paste0(parameters$directory,"/results/loo/ageing_and_neuropathy/cpt_", str_remove_all(parameters$model,".stan"), ".rds"))
  rm(loo)
  gc()
  loo<-fit$loo(cores=4,variables='log_lik_cpt',moment_match=T)
  saveRDS(loo,paste0(parameters$directory,"/results/loo/ageing_and_neuropathy/MM_cpt_", str_remove_all(parameters$model,".stan"), ".rds"))
  rm(loo)
  gc()
  loo<-fit$loo(cores=4,variables='log_lik_hpt')
  saveRDS(loo,paste0(parameters$directory,"/results/loo/ageing_and_neuropathy/hpt_", str_remove_all(parameters$model,".stan"), ".rds"))
  rm(loo)
  gc()
  loo<-fit$loo(cores=4,variables='log_lik_hpt',moment_match=T)
  saveRDS(loo,paste0(parameters$directory,"/results/loo/ageing_and_neuropathy/MM_hpt_", str_remove_all(parameters$model,".stan"), ".rds"))
  rm(loo)
  gc()
  sink(NULL)
  sink(NULL)  
...
init_f_Gaussian_unconstrained_threshold_ageing_and_neuropathy<-function(){
  init<-list(
    mu=c(
      rnorm(1,0.67,0.09*4),
      rnorm(1,4.33,0.31*4),
...
init_f_Gaussian_constrained_threshold_ageing_and_neuropathy<-function(){
  init<-list(
    mu=c(
      rnorm(1,-0.52,0.15*4),
      rnorm(1,1.35,0.06*4),
...
init_f_Quick_constrained_threshold_ageing_and_neuropathy<-function(){
  init<-list(
    mu=c(
      rnorm(1,-0.57,0.17*4),
      rnorm(1,1.35,0.09*4),
...
init_f_Gaussian_unconstrained_threshold_ageing_and_neuropathy_GLN<-function(){
  init<-list(
    mu=c(
      rnorm(1,0.67,0.09*4),
      rnorm(1,4.33,0.31*4),
...
init_f_Gaussian_constrained_threshold_ageing_and_neuropathy_GLN<-function(){
  init<-list(
    mu=c(
      rnorm(1,-0.52,0.15*4),
      rnorm(1,1.35,0.06*4),
...
init_f_Quick_constrained_threshold_ageing_and_neuropathy_GLN<-function(){
  init<-list(
    mu=c(
      rnorm(1,-0.57,0.17*4),
      rnorm(1,1.35,0.09*4),
...
init_f_Gaussian_unconstrained_threshold_ageing_and_neuropathy_GLN_NI<-function(){
  init<-list(
    mu=c(
      rnorm(1,0.67,0.09*4),
      rnorm(1,4.33,0.31*4),
...
init_f_Gaussian_constrained_threshold_ageing_and_neuropathy_GLN_NI<-function(){
  init<-list(
    mu=c(
      rnorm(1,-0.52,0.15*4),
      rnorm(1,1.35,0.06*4),
...
init_f_Quick_constrained_threshold_ageing_and_neuropathy_GLN_NI<-function(){
  init<-list(
    mu=c(
      rnorm(1,-0.57,0.17*4),
      rnorm(1,1.35,0.09*4),
...
init_f_Gaussian_unconstrained_threshold_ageing_and_neuropathy_GLN_NI_H<-function(){
  init<-list(
    mu=c(
      rnorm(1,0.67,0.09*4),
      rnorm(1,4.33,0.31*4),
...
init_f_Gaussian_constrained_threshold_ageing_and_neuropathy_GLN_NI_H<-function(){
  init<-list(
    mu=c(
      rnorm(1,-0.52,0.15*4),
      rnorm(1,1.35,0.06*4),
...
init_f_Quick_constrained_threshold_ageing_and_neuropathy_GLN_NI_H<-function(){
  init<-list(
    mu=c(
      rnorm(1,-0.57,0.17*4),
      rnorm(1,1.35,0.09*4),
...
models=c(
  'Gaussian_unconstrained_threshold_ageing_and_neuropathy.stan',
  'Gaussian_constrained_threshold_ageing_and_neuropathy.stan',
  'Quick_constrained_threshold_ageing_and_neuropathy.stan',

  'Gaussian_unconstrained_threshold_ageing_and_neuropathy_GLN.stan',
  'Gaussian_constrained_threshold_ageing_and_neuropathy_GLN.stan',
  'Quick_constrained_threshold_ageing_and_neuropathy_GLN.stan',

  'Gaussian_unconstrained_threshold_ageing_and_neuropathy_GLN_NI.stan',
  'Gaussian_constrained_threshold_ageing_and_neuropathy_GLN_NI.stan',
  'Quick_constrained_threshold_ageing_and_neuropathy_GLN_NI.stan',

  'Gaussian_unconstrained_threshold_ageing_and_neuropathy_GLN_NI_H.stan',
  'Gaussian_constrained_threshold_ageing_and_neuropathy_GLN_NI_H.stan',
  'Quick_constrained_threshold_ageing_and_neuropathy_GLN_NI_H.stan'
)
