...
transformed data{
  ...
  int K=(C*I+U)*3;               //Number of main effects
...
transformed parameters{
  ...
  alpha[1] = exp(eta_0_0[1,] + mu[T+1] .* h + (mu[2*T+1]) .*ra);
  alpha[2] = exp(eta_0_0[2,] + mu[T+2] .* h + (mu[2*T+2]) .*ra);
  alpha[3] = exp(eta_0_0[3,] + mu[T+3] .* h + (mu[2*T+3]) .*ra);
  alpha[4] = exp(eta_0_0[4,] + mu[T+4] .* h + (mu[2*T+4]) .*ra);
  beta[1] = exp(eta_0_0[5,] + mu[T+5] .* h + (mu[2*T+5]) .*ra);
  beta[2] = exp(eta_0_0[6,] + mu[T+6] .* h + (mu[2*T+6]) .*ra);
  beta[3] = exp(eta_0_0[7,] + mu[T+7] .* h + (mu[2*T+7]) .*ra);
  beta[4] = exp(eta_0_0[8,] + mu[T+8] .* h + (mu[2*T+8]) .*ra);
  gamma = .5*inv_logit(eta_0_0[9,] + mu[T+9] .* h + (mu[2*T+9]) .*ra);
  lambda = .5*inv_logit(eta_0_0[10,] + mu[T+10] .* h + (mu[2*T+10]) .*ra);
...
generated quantities{
  ...
  matrix[T,P] partial_residuals = diag_pre_multiply(rep_vector(1,T), L) * z;
}
