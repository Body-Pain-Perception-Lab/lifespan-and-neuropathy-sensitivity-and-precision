...
// Guess and lapse rate treatment: nuisance (random intercepts but no age or neuropathy effects)
...
transformed data{
  ...
  int K=(C*I+U)+C*I*3;           //mber of main effects
  int T=C*I*2+U;                 //Number of random effects
...
transformed parameters{
  ...
  matrix[10,P] eta_0_0;           //All participants intercept
  matrix[8,P] eta_0_1;            //Difference between patients and controls intercepts
  ...
  for(idx in 1:8){
    eta_0_1[idx,] = mu[idx+10] + delta[idx+10,];
  }
  
  alpha[1] = exp(eta_0_0[1,] + eta_0_1[1,] .* h + (mu[T+1] + mu[T+9] .* h) .*ra);
  alpha[2] = exp(eta_0_0[2,] + eta_0_1[2,] .* h + (mu[T+2] + mu[T+10] .* h) .*ra);
  alpha[3] = exp(eta_0_0[3,] + eta_0_1[3,] .* h + (mu[T+3] + mu[T+11] .* h) .*ra);
  alpha[4] = exp(eta_0_0[4,] + eta_0_1[4,] .* h + (mu[T+4] + mu[T+12] .* h) .*ra);
  beta[1] = exp(eta_0_0[5,] + eta_0_1[5,] .* h + (mu[T+5] + mu[T+13] .* h) .*ra);
  beta[2] = exp(eta_0_0[6,] + eta_0_1[6,] .* h + (mu[T+6] + mu[T+14] .* h) .*ra);
  beta[3] = exp(eta_0_0[7,] + eta_0_1[7,] .* h + (mu[T+7] + mu[T+15] .* h) .*ra);
  beta[4] = exp(eta_0_0[8,] + eta_0_1[8,] .* h + (mu[T+8] + mu[T+16] .* h) .*ra);
  gamma = .5*inv_logit(eta_0_0[9,]);
  lambda = .5*inv_logit(eta_0_0[10,]);
...
model{
  ...
  mu[18] ~ normal(0,0.85);
  //Eta_1_0
  mu[19] ~ normal(0,0.92/60);
  mu[20] ~ normal(0,0.53/60);
  mu[21] ~ normal(0,0.38/60);
  mu[22] ~ normal(0,0.17/60);
  mu[23] ~ normal(0,1.09/60);
  mu[24] ~ normal(0,0.75/60);
  mu[25] ~ normal(0,0.53/60);
  mu[26] ~ normal(0,0.85/60);
  //Eta_1_1
  mu[27] ~ normal(0,0.92/60);
  mu[28] ~ normal(0,0.53/60);
  mu[29] ~ normal(0,0.38/60);
  mu[30] ~ normal(0,0.17/60);
  mu[31] ~ normal(0,1.09/60);
  mu[32] ~ normal(0,0.75/60);
  mu[33] ~ normal(0,0.53/60);
  mu[34] ~ normal(0,0.85/60);
