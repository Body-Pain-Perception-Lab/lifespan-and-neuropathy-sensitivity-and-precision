// Stan model for the ageing and neuropathy dataset (model 3 used in the paper)
...
// Random slope for neuropathy : no
...
transformed data{
  ...
  int K=C*I*3+U;                 //Number of main effects
...
transformed parameters{
  ...
  alpha[1] = exp(eta_0_0[1,] + mu[T+1] .* h + mu[T+C*I+1].*ra);
  alpha[2] = exp(eta_0_0[2,] + mu[T+2] .* h + mu[T+C*I+2] .*ra);
  alpha[3] = exp(eta_0_0[3,] + mu[T+3] .* h + mu[T+C*I+3] .*ra);
  alpha[4] = exp(eta_0_0[4,] + mu[T+4] .* h + mu[T+C*I+4] .*ra);
  beta[1] = exp(eta_0_0[5,] + mu[T+5] .* h + mu[T+C*I+5] .*ra);
  beta[2] = exp(eta_0_0[6,] + mu[T+6] .* h + mu[T+C*I+6] .*ra);
  beta[3] = exp(eta_0_0[7,] + mu[T+7] .* h + mu[T+C*I+7] .*ra);
  beta[4] = exp(eta_0_0[8,] + mu[T+8] .* h + mu[T+C*I+8] .*ra);
