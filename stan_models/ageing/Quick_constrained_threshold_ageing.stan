// Stan model for the ageing only dataset
...
transformed parameters{
  ...
  gamma = .5*inv_logit(kappa_0[9,]+mu[K+9].*ra);
  lambda = .5*inv_logit(kappa_0[10,]+mu[K+10].*ra);
...
model{
  ...
  //Kappa_1
  mu[11] ~ normal(0,1.02/60);
  mu[12] ~ normal(0,0.53/60);
  mu[13] ~ normal(0,0.41/60);
  mu[14] ~ normal(0,0.16/60);
  mu[15] ~ normal(0,0.39/60);
  mu[16] ~ normal(0,0.73/60);
  mu[17] ~ normal(0,0.33/60);
  mu[18] ~ normal(0,0.97/60);
  mu[19] ~ normal(0,0.86/60);
  mu[20] ~ normal(0,0.39/60);
...
generated quantities{
  ...
  for(n in 1:N_cdt){
    log_lik_cdt[n] = bernoulli_lpmf(y_cdt[n]|theta_cdt[n]);
  }
  for(n in 1:N_wdt){
    log_lik_wdt[n] = bernoulli_lpmf(y_wdt[n]|theta_wdt[n]);
  }
  for(n in 1:N_cpt){
    log_lik_cpt[n] = bernoulli_lpmf(y_cpt[n]|theta_cpt[n]);
  }
  for(n in 1:N_hpt){
    log_lik_hpt[n] = bernoulli_lpmf(y_hpt[n]|theta_hpt[n]);
  }
